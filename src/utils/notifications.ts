...
export function notifications(data: any) {
    if (data.source === Notif_Source.Sales_Invoice) {
      return `${data.orgBranch} has created a sales invoice for ${data.totalCost} due on ${formatDateAndTime(data.createdAt).formattedDate}.`;
    } 
    ...
    if(data.source===Notif_Source.Expense_Invoice){
        return `${data.productName} has created an expense invoice for ${data.totalCost} due on ${formatDateAndTime(data.createdAt).formattedDate}. `
    }
