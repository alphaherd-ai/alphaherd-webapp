...
export const PUT = async (req: NextRequest, { params }: { params: { invoice: string } }) => {
    ...
    try {
        ...
        const { receiptNo, ...otherData } = body;
        if (otherData.amountPaid) {
            otherData.amountPaid = parseInt(otherData.amountPaid, 10); // Ensure it's an integer
        }


        const CheckRecordTransaction = await prismaClient.transactions.findFirst({
            where: {
                receiptNo: receiptNo
            }
        })
        ...
        const updatedRecordTransaction = await prismaClient.transactions.update({
