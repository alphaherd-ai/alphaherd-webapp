...
const DownloadPopup = ({ onClose, distributors }:any) => {

  const appState = useAppSelector((state) => state.app)
  const [data, setData] = useState(distributors);
  ...
  const handleFilter = (start:any, end:any) => {
    const filteredData = distributors.filter((item:any) => {
  ...
  const downloadPDF = () => {
    convertImageToBase64(logo, (base64Image: any) => {
      ...
      data.forEach((item: any) => {
        ...
        tableRows.push(distributorsData);
      });
