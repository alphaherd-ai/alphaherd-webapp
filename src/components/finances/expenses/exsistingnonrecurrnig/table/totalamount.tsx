...
<div className="w-full  px-6 bg-white rounded-bl-md rounded-br-md justify-between items-center flex border border-t-0 border-solid border-borderGrey">
    ...
    {!(otherData?.status === 'Cancelled') &&
        <>
            {popup && <EditRecordTransactionPopup onClose={onClose} editTransaction={transaction} type={"exsistingnonrecurring"} balanceDue={balanceDue} />}
            {showConfirmation && <CancellationPopup setShowConfirmation={setShowConfirmation} editTransaction={transaction} type={"exsistingnonrecurring"} balanceDue={balanceDue} />}
        </>
    }
        </div>  
