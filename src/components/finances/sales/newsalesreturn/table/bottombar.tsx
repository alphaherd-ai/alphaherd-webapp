...
const NewsalesReturnBottomBar = ({ invoiceData }: any) => {
    ...
    const handleSubmit = async () => {
        ...
        const data = {
            ...
            status: allData.totalAmountData.totalCost === (balanceDue) ? `Credited:₹${(balanceDue).toFixed(2)}` : balanceDue <= -1 ? `You’re owed: ₹${parseFloat((-1 * balanceDue).toString()).toFixed(2)}` : balanceDue >= 1 ? `You owe: ₹${parseFloat((balanceDue).toString()).toFixed(2)}` : 'Closed',
