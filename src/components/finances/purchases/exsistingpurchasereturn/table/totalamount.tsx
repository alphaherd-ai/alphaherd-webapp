...
// const ExsistingPurcaseReturnTotalAmount = ({ otherData, isLoading, items, checkedItems }: any) => {
...
// const selectedItems = Array.isArray(items) ? items.filter((item: any) => checkedItems[item.id]) : [];

// const subtotal = selectedItems.reduce((acc: number, item: any) => acc + item.unitPrice * item.quantity, 0);

//     const overallDiscountAmount = subtotal * (otherData?.overallDiscount || 0);

//     const grandTotal = subtotal - overallDiscountAmount + (otherData?.shipping || 0) + (otherData?.adjustment || 0);

//     const balanceDue = grandTotal - totalPaidAmount + totalAmountPay;
...
//           <div className="text-right text-gray-500 text-base font-bold">₹{subtotal.toFixed(2)}</div>
//         </div>
...
//           <div className="text-right text-textGreen text-base font-bold">₹{grandTotal.toFixed(2)}</div>
//         </div>
//       </div>
            ...
            <div className="flex gap-4 w-full box-border bg-gray-100 pt-[20px] pb-[20px]">
                ...
                <div className='w-1/2 '>
                    <div className="w-full mr-4 flex flex-col ">
                        ...
                        <div className='w-full mr-4 mt-4 flex flex-col border border-solid border-borderGrey rounded-[5px]'>
                            <div className="w-full  px-6 py-4 bg-white rounded-tl-md rounded-tr-md justify-between items-center gap-6 flex border-0 border-b border-solid border-borderGrey">
                                <div className="text-gray-500 text-xl font-medium ">Payments</div>
